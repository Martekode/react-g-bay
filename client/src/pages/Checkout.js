...
function Checkout() {
  ...
  const [userIdFromDB, setUserIdFromDb] = React.useState("");
  const { user } = useAuth0();
  React.useEffect(() => {
    const fetchUserID = async (email) => {
      const BASE_URL = "http://localhost:3050/";
      const endpoint = "api/user/email/";
      const response = await axios.get(BASE_URL + endpoint + email);
      setUserIdFromDb(response.data[0].id);
    };
  ...
  return (
    <div>
      <p className="flex justify-center items-center text-6xl mt-16 font-bangers">
        Checkout
      </p>
      <div className="flex flex-row mt-12 font-bangers text-xl">
        {CheckoutArrayValue.map((product, i) => {
          ...
          return (
            <div className="flex flex-col md:flex-row md:max-w-xl rounded-lg bg-backgroundTwo shadow-lg scale-75 bg-product">
              <img
              ...
              <div className="p-12 flex flex-col items-center">
                <h5 key={i} className="text-gray-900 text-3xl font-medium mb-2">
                  {product.name}
                </h5>
                ...
                <button
                  ...
                  className="border-2 border-solid mt-6 hover:bg-hovers p-2 w-40 h-20 flex justify-center items-center rounded-full"
                >
                  Buy this item right now!
                </button>
              </div>
            </div>
          );
      ...
      <p className="flex justify-center items-center mb-12 text-3xl font-bangers">
        Total price :{" "}
        {totalPrice
          ? totalPrice
